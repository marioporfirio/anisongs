...
async function getAnimeDetails(slug: string): Promise<AnimeDetail> {
    ...
    const response = await fetch(API_URL, { next: { revalidate: 3600 } }); // Cache de 1 hora
...
export default async function AnimeDetailPage({ params }: { params: { slug: string } }) {
  ...
  // Lógica para buscar as avaliações de cada tema
  const themesWithRatings = await Promise.all(
    anime.animethemes.map(async (theme): Promise<ThemeWithRating> => {
      const { data: avgData } = await supabase.rpc('get_theme_average_rating', { p_anime_slug: params.slug, p_theme_slug: theme.slug });
      let userScore = null;
      if (user) {
        const { data: userRating } = await supabase.from('ratings').select('score').eq('user_id', user.id).eq('anime_slug', params.slug).eq('theme_slug', theme.slug).single();
  ...
  // Lógica para ordenar e agrupar os temas
  const openings: ThemeWithRating[] = [];
  const endings: ThemeWithRating[] = [];
  const others: ThemeWithRating[] = [];
  const getThemeNumber = (slug: string) => { const match = slug.match(/\d+/); return match ? parseInt(match[0], 10) : Infinity; };
